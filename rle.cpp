    ...
    while (i < length) {
        ...
        if (count > 0) {
            for (int j = 0; j < count; ++j)
                write(original, &current[COUNT_SIZE], SYMBOL_SIZE);
        ...
        write(original, &current[COUNT_SIZE], SYMBOL_SIZE * count);
    }
}
...
void encode2(const byte *original, int length, byte_array &compressed) {
    ...
    for (; i < length; i += 1 * SYMBOL_SIZE, j += 1 * SYMBOL_SIZE, ++cnt1)
        if (!equal(&original[j], &original[i], SYMBOL_SIZE)) {
            ...
            if (cnt2 != 0) {
                write_nseq_2bytes(compressed, &original[begin], cnt2, SYMBOL_SIZE);
                cnt2 = 0;
            }
            begin = i;
            write_seq_2bytes(compressed, &original[j], cnt1, SYMBOL_SIZE);
            cnt1 = 0;
        }
